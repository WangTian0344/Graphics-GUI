#include "dda.h"
#include <QDebug>
DDA::DDA(QWidget *parent) : QWidget(parent)
{
    this->setWid();
    this->resize(800,800);//组件大小
}

void DDA::setWid()//设置组件
{
    this->label[0]=new QLabel("DDA画线算法",this);
...
void DDA::mousePressEvent(QMouseEvent *event)//鼠标点击坐标系，坐标系显示点
{
    //鼠标左键单击且点击位置为坐标系内
...
void DDA::transform()
{
    ...
    if(this->trans->btnbox->checkedId()==0)
        ...
        sym_type=2;
    this->trans->empty();
    this->coo->transform(translate_x,translate_y,angle,sym_type,0);
}
